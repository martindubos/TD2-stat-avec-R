...
représentation de la variable Salary en fonction de Neuroticism sous la forme d’un nuage de points

```{r}
ggplot(data = data, 
       mapping = aes(x= Neuroticism, y=Salary))+
  geom_point()+
...
```{r}
cor(data$Salary,data$Neuroticism)
```
...
```{r}
data %>% 
  ggplot(aes(Neuroticism,Salary,color = Education))+
  geom_point()+
  geom_smooth(method="lm")
```
